...
if not os.path.exists(log_file):
    with open(log_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Timestamp', 'Width (cm)', 'Height (cm)', 'Weapon Detected'])

# Email sending function
def send_email(image_path, width, height):
    sender_email = "topjrealworld@gmail.com"
    receiver_email = "godwinfx19@gmail.com"
    ...
    # Email body with added information (height, width)
...
# Function to send email asynchronously with delay
def send_email_async(image_path, width, height):
    global email_last_sent_time
    current_time = time.time()
    
    # Check if enough time has passed since the last email
    if current_time - email_last_sent_time >= email_cooldown:
        # Send email in a new thread
        thread = threading.Thread(target=send_email, args=(image_path, width, height))
        thread.start()
        
        # Update the last sent time
        email_last_sent_time = current_time
    else:
        print(f"[INFO] Skipping email send. Cooldown period not over. Try again in {int(email_cooldown - (current_time - email_last_sent_time))} seconds.")
...
# Adding a 5-minute (300 seconds) cooldown period between email alerts
email_last_sent_time = 0
email_cooldown = 60  # 5 minutes cooldown in seconds

# New variables for black screen detection
last_black_time = 0
black_screen_threshold = 8  # seconds
alert_delay_time = 5 # seconds
last_alert_time = 0
...
while cap.isOpened():
    ...
    # Check if the frame is black (i.e., no movement)
    if np.mean(frame) < 30:  # Threshold for black frame detection
        if time.time() - last_black_time > black_screen_threshold:
            if time.time() - last_alert_time > alert_delay_time:
                # Trigger alert if frame is black for more than 8 seconds
                timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
                image_filename = f"{capture_dir}/black_alert_{timestamp}.jpg"
                cv2.imwrite(image_filename, frame)  # Save image
                print(f"[ALERT] Black screen detected! Image saved: {image_filename}")

                # Send email
                send_email_async(image_filename, 0, 0)
                last_alert_time = time.time()  # Update the time of the last alert

            last_black_time = time.time()  # Reset the black frame time
    ...
    if result.pose_landmarks and is_full_body_detected(result.pose_landmarks.landmark) and weapon_detected:
        ...
        with open(log_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([timestamp, width_in_cm, height_in_cm, "Yes"])

        print(f"[ALERT] Weapon detected! Image saved: {image_filename}")

        # Send email asynchronously with all info
        send_email_async(image_filename, width_in_cm, height_in_cm)

...
# Variable to check if the frame is black or blurred
last_non_black_time = time.time()
last_non_blurred_time = time.time()

def is_frame_blurry(frame):
    """Check if the frame is blurry by computing the Laplacian variance."""
...
while cap.isOpened():
    ...
    # Check if the frame is black (no content) or blurred
    if np.all(frame == 0) or is_frame_blurry(frame):
        if time.time() - last_non_black_time > 8 or time.time() - last_non_blurred_time > 8:
            # Send alert if the frame remains black or blurred for more than 8 seconds
            timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
            alert_image = f"{capture_dir}/tamper_frame_alert_{timestamp}.jpg"
            cv2.imwrite(alert_image, frame)
            send_email_async(alert_image, 0, 0, "Unknown", "Unknown")
            print("[ALERT] Tamper or blurry frame detected! Email sent.")
    else:
        last_non_black_time = time.time()
        last_non_blurred_time = time.time()
    ...
    if result.pose_landmarks and is_full_body_detected(result.pose_landmarks) and weapon_detected:
